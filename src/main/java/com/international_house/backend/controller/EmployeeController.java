...
public class EmployeeController {
    ...
    public ResponseEntity<BaseResponseDto> addNewEmployee(@RequestBody Employee employee) {
        ...
        return ResponseEntity.ok(BaseResponseDto.builder().message("Employee added successfully!").data(newEmployee).build());
    }

    @GetMapping
    public ResponseEntity<BaseResponseDto> getEmployees() {
        List<Employee> employees = employeeService.getEmployees();
        return ResponseEntity.ok(BaseResponseDto.builder().message("Employees retrieved successfully!").data(employees).build());
    }

    @GetMapping("/{id}")
    public ResponseEntity<BaseResponseDto> getEmployeeById(@PathVariable UUID id) {
        Employee employee = employeeService.getEmployeeById(id);
        return ResponseEntity.ok(BaseResponseDto.builder().message("Employee retrieved successfully!").data(employee).build());
    }
